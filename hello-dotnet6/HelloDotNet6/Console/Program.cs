...
namespace ConsoleTester
{
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            while (keepRunning)
            {
                Console.WriteLine("Input 'weather ttt 1 1 ENTER' to execute WebAPIAhttp weatherforecast route.");
                Console.WriteLine("OR other commands TBD.");
                ...
                if (keepRunning)
                {
                    string requestDtoToWebAPI;
                    string responseDtoFromWebAPI;
                    ...
                    switch (cmdNArgs.Cmd)
                    {
                        case "weather":
                            ...
                            requestDtoToWebAPI = cmdNArgs.Arg1;
                            Int32.TryParse(cmdNArgs.Arg2, out nRequests); 
                            if(nRequests <= 0) nRequests = 1;
                            Int32.TryParse(cmdNArgs.Arg3, out delayMsec);                            
                            
                            string weatherforecastFullUrl = ServiceEndpointUrls.WebAPIAhttpBaseUrl +
                                                            ServiceEndpointUrls.WeatherForecast;

                            for (int i = 1; i < nRequests + 1; i++)
                            {
                                HttpResponseMessage response = await m_HttpClient.GetAsync(weatherforecastFullUrl);
                                responseDtoFromWebAPI = await response.Content.ReadAsStringAsync();

                                // Optionally can encapsulate url strings above into a proxy that returns a specific class
                                // via generics.  Not yet implemented.
                                //replyDtoFromWebAPI = await webApiAProxy.WeatherForecast(requestDtoToWebAPI);                                

                                Console.WriteLine($"Made Request number {i} of {nRequests} requests to make.");

                                // Below can be useful.  But not necessary.
                                // Don't clutter up the display unless there are errors.
                                //if (replyDtoFromWebAPI.Contains("Error"))
                                //{
                                //    resultsMsg = $"Service response = {replyDtoFromWebAPI}\n";
                                //    Console.WriteLine(resultsMsg);
                                //}
                                Console.WriteLine($"Service response = {responseDtoFromWebAPI}\n");
                                await Task.Delay(delayMsec);
                            }  

                            break;

                        // TODO Add code here, using Http Post to ServiceEndpointUrls.WebAPIAhttpUrl send
                        // service requests and their data in an RPC style to WebAPIAhttp.  Need to append the
                        // tail end route to the base URL.
        ...
        private class BasicHttpRequestDTO
        {
            public string StringRequest { get; set;}
        }
        private static string ComposeRequestPayload(int i, string uiEventPayload)
        {
        ...
        private static ParsedUserInput ParseUserInput(string userInput)
        {
            ...
            try
            {
                ...
                // Command   Serialized DTO        Number of Iterations    Millisecond delay between iterations
                // weather   some data-payload     100 This must be >= 1   1000 This can be 0
                // 
                cmdNArgs.Cmd = inputItems[0];  // Command
                cmdNArgs.Arg1 = inputItems[1]; // Serialized DTO
                cmdNArgs.Arg2 = inputItems[2]; // Number of iterations (requests).  Defaults to 1.
                cmdNArgs.Arg3 = inputItems[3]; // Number of MILLISECONDS to delay between iterations.  Defaults to 0.
                cmdNArgs.Arg4 = inputItems[4]; // For future use.
                cmdNArgs.Arg5 = inputItems[5]; // For future use.
                cmdNArgs.Arg6 = inputItems[6]; // For future use.
            }
            catch (Exception ex)
            {
            }
        ...
        // George 5/7/22  This can be useful when doing posts.  I wrote it for some of my own code.
        private async Task<HttpResponseMessage> SendHttpRequest<T>(T requestDto, string fullTestUrl)
                                                                                     where T : class
        {
            // Below is used ONLY to view and copy JSON strings for Postman via copy/paste.
            // string json = JsonConvert.SerializeObject(requestDto);
            // System.Text.Json is used throughout this solution, thus the above comment applies below as well.
            string jsonString = System.Text.Json.JsonSerializer.Serialize(requestDto);

            // Below uses System.Text.Json
            var response = await m_HttpClient.PostAsJsonAsync(fullTestUrl, requestDto);
